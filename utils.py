...
def pad_single(image, new_shape, constant = 0):
    y_size, x_size = image.shape
    ...
    new_image = np.pad(image, (y_pad, x_pad), constant_values=constant)
    return new_image
...
def pad_images_np(source, target, constant = 0):
    ...
    padded_source = pad_single(source, new_shape, constant)
    padded_target = pad_single(target, new_shape, constant)
...
def load_pair(case_id, dataset_path, load_masks=False):
    ...
    if load_masks:
        source_mask_path = os.path.join(base_path, "label1_origin.jpg")
        target_mask_path = os.path.join(base_path, "label2_origin.jpg")
...
if __name__ == '__main__':
    ...
    output_path = 'E:\data\ANHIR_Final'  # TO DEFINE
    dhr_params = dict()
    ...
    affine_registration_params['model_path'] = 'E:/data/ANHIR_Pretrained/affine_model_512'  # TO DEFINE
    ...
    nonrigid_registration_params['model_path'] = "E:/data/ANHIR_Pretrained/nonrigid_2048"  # TO DEFINE
    dhr_params['nonrigid_registration_params'] = nonrigid_registration_params
    segmentation_params = dict()
    segmentation_params['model_path'] = 'E:/data/ANHIR_Pretrained/segmentation_model_512'  # TO DEFINE
    dhr_params['segmentation_params'] = segmentation_params
    load_masks = False
    ###
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    source_path = "E:/data/ANHIR_Final/0/source.mha"
    target_path = "E:/data/ANHIR_Final/0/target.mha"
    source = sitk.GetArrayFromImage(sitk.ReadImage(source_path))
    target = sitk.GetArrayFromImage(sitk.ReadImage(target_path))
    # TODO 对 source 和 target 下采样
    source = resample_image(source, 8)
    target = resample_image(target, 8)
    ...
    hog_source, vis_source_image = ft.hog(source.cpu().numpy(), orientations=12, pixels_per_cell=[40, 40],
                                          cells_per_block=[2, 2], visualize=True, feature_vector=False)
    hog_source, vis_transformed_image = ft.hog(transformed_target.cpu().numpy(),orientations=12,pixels_per_cell=[40,40],cells_per_block=[2,2],visualize=True,feature_vector=False)
    hog_target, vis_target_image = ft.hog(target.cpu().numpy(), orientations=12,pixels_per_cell=[40,40],cells_per_block=[2,2],visualize=True,feature_vector=False)


    vis_source_image = torch.from_numpy(vis_source_image).to(device)
    vis_target_image = torch.from_numpy(vis_target_image).to(device)


    vis_transformed_image_1 = warp_tensor(vis_target_image, displacement_field, device=device)
    vis_transformed_image_2 = torch.from_numpy(vis_transformed_image).to(device)


    print(cost_functions.ncc_loss_global(source, target))
    print(cost_functions.ncc_loss_global(source, transformed_target))
    print(cost_functions.ncc_loss_global(vis_source_image, vis_target_image))
    # TODO displacement_field到底是对哪张图像做的
    print(cost_functions.ncc_loss_global(vis_source_image, vis_transformed_image_1))
    print(cost_functions.ncc_loss_global(vis_source_image, vis_transformed_image_2))

    vis_source_image = vis_source_image.cpu().numpy()
    vis_target_image = vis_target_image.cpu().numpy()
    vis_transformed_image_1 = vis_transformed_image_1.cpu().numpy()
    vis_transformed_image_2 = vis_transformed_image_2.cpu().numpy()

    plt.imshow(vis_source_image, cmap=plt.cm.gray)
    plt.show()
    plt.imshow(vis_target_image, cmap=plt.cm.gray)
    plt.show()
    plt.imshow(vis_transformed_image_1, cmap=plt.cm.gray)
    plt.show()
    plt.imshow(vis_transformed_image_2, cmap=plt.cm.gray)
    plt.show()